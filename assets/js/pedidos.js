...
const COMPANY_KEY = KEYS.COMPANY;
let orders = [];
let draftItems = [];
...
function getCompanySettings() {
    ...
    try {
        ...
        return {
            ...fallback,
            ...stored,
            name: String(stored.name || fallback.name).trim() || fallback.name
        };
    } catch {
        return fallback;
    }
...
function configureCompanyOnTicket() {
    const company = getCompanySettings();
    setText("ticketCompanyName", company.name.toUpperCase());

    const meta = document.getElementById("ticketCompanyMeta");
    if (!meta) return;
...
async function deleteOrder(id) {
    ...
    const confirmDelete =
        await confirmAction({
            title:
                `Excluir pedido #${formatOrderNumber(order.number)}?`,
            message:
                `O pedido de ${order.client} será removido do histórico.`,
            details:
                "A movimentação vinculada no Controle Geral também será removida.",
            confirmLabel:
                "Excluir pedido",
            cancelLabel:
                "Cancelar",
